package com.zhs.imgselect.share.view;
...
//            mVibrator.vibrate(50); //震动一下
            if((mDragPosition==PostImagesActivity.ADD_POSITION)||(mDragPosition==PostImagesActivity.ADD_POSITION-1)){
                return;
            }
            mStartDragItemView.setVisibility(View.INVISIBLE);//隐藏该item

            if (onChanageListener != null) {
                onChanageListener.onShowDel(mDragPosition);
            }
            //根据我们按下的点显示item镜像
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        switch (ev.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                Log.d("wwq","mDragPositon: "+mDragPosition+"       "+PostImagesActivity.ADD_POSITION);
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if((mDragPosition==PostImagesActivity.ADD_POSITION-1)||(mDragPosition==0&&PostImagesActivity.ADD_POSITION==0)){
                    return false;
                }
    ...
    private void createDragImage(Bitmap bitmap, int downX, int downY) {
        ...
        mWindowLayoutParams.alpha = 0.95f; //透明度
    ...
    private void onDragItem(int moveX, int moveY) {
        ...
        int aaa = height - dip2px(context, dpValue);
        Log.d("wwq","123 :"+((moveY - (mPoint2ItemTop / 2) + mOffset2Top + mStatusHeight) > aaa));
        if ((moveY - (mPoint2ItemTop / 2) + mOffset2Top + mStatusHeight) > aaa) {
            if(onChanageListener!=null){
                onChanageListener.onViewSelected(1);
            }
        }else{
            if(onChanageListener!=null){
                onChanageListener.onViewSelected(0);
            }
        }
    ...
    public void setDP(float dpValue) {
        this.dpValue = dpValue;
    }

    private void onDragItem1(int moveX, int moveY) {
        mWindowLayoutParams.x = moveX - mPoint2ItemLeft + mOffset2Left;
        mWindowLayoutParams.y = moveY - mPoint2ItemTop + mOffset2Top - mStatusHeight;
        mWindowManager.updateViewLayout(mDragImageView, mWindowLayoutParams); //更新镜像的位置
        ...
        int aaa = height - dip2px(context, dpValue);
        Log.d("wwq","temp:"+((moveY - (mPoint2ItemTop / 2) + mOffset2Top + mStatusHeight) > aaa));
        if ((moveY - (mPoint2ItemTop / 2) + mOffset2Top + mStatusHeight) > aaa) {
            if (onChanageListener != null) {
                onChanageListener.onDel(mDragPosition);
            }
        } else {
            if (onChanageListener != null) {
                onChanageListener.onDel(-1);
            }
        }
    ...
    private void onSwapItem(int moveX, int moveY) {
        ...
        if(tempPosition==PostImagesActivity.ADD_POSITION-1){
            return;
        }
